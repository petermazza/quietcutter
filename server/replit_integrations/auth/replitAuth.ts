...
const getOidcConfig = memoize(
  async () => {
    ...
    if (!replId) {
      throw new Error('REPL_ID not set - Replit auth not available');
    }
...
export function getSession() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    console.log("DATABASE_URL not set, using in-memory session store (sessions will not persist across restarts)");
  }
